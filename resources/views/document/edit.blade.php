...
@section('content')
    ...
    <div class="editor-main">
        ...
        <div class="editor-area" id="editor-area" style="padding:20px;display:flex;justify-content:center;">
            <div style="position:relative;width:100%;max-width:900px;display:flex;flex:1;">
                <textarea id="editor" class="txt-editor" placeholder="Mulai mengetik di sini..." {{ $bisaEdit ? '' : 'readonly' }}
                    data-update-url="{{ route('document.update', $document->id) }}"
                    data-broadcast-url="{{ route('document.broadcast', $document->id) }}"
                    data-broadcast-cursor-url="{{ route('document.broadcastCursor', $document->id) }}"
            ...
            <div class="version-list">
                @forelse($riwayatVersi as $versi)
                    <div class="version-item">
                        <div class="v-meta">{{ $versi->created_at->format('d M Y, H:i') }} &bull; oleh
                            {{ $versi->savedBy->name }}</div>
    ...
    <!-- Minimalist Floating Conflict Card -->
    <div class="conflict-card" id="conflict-card">
        <div class="conflict-card-msg">
            ⚠️ <strong>Konflik ketikan</strong> dengan <span id="conflict-card-user-name">User</span>
        </div>
        <div class="conflict-card-details">
            Mereka: <span id="conflict-card-user-edit">...</span> | Anda: <span id="conflict-card-my-edit">...</span>
        </div>
        <div class="conflict-card-actions">
            <button class="btn-conflict" id="btn-resolve-theirs">Mereka</button>
            <button class="btn-conflict" id="btn-resolve-mine">Saya</button>
            <button class="btn-conflict btn-conflict-merge" id="btn-resolve-merge">Gabung</button>
        </div>
    </div>
            ...
            <div class="modal-box">
                ...
                <div class="modal-body">
                    ...
                    <div class="share-list" id="share-list">
                        ...
                        @empty
                            <div id="no-shares-msg" style="font-size:13px;color:#bbb;text-align:center;padding:16px 0;">
                                Belum ada yang diberi akses.</div>
                        @endforelse
                    </div>
                </div>
            </div>
